...
#
# Desc: helper for spawning models in gazebo
# Author: John Hsu, Dave Coleman, Adam Allevato
#
...
class SpawnModel():
    ...
    By using the Python API, you can also directly supply the SDF model syntax
        '''

    def parseUserInputs(self):
        # get goal from commandline
        for i in range(0, len(sys.argv)):
            if (sys.argv[i] == '-h' or sys.argv[i] == '--help' or
                    sys.argv[i] == '-help'):
                self.usage()
                sys.exit(1)
            ...
            if sys.argv[i] == '-sdf' or sys.argv[i] == '-gazebo':
                if self.urdf_format == True:
                    ...
                    if sys.argv[i] == '-gazebo':
                        ...
                        warnings.warn("Deprecated: the -gazebo tag is now -sdf",
                                      DeprecationWarning)
    ...
    def callSpawnService(self):
        ...
        if not self.wait_for_model == "":
            rospy.Subscriber("%s/model_states" % (self.gazebo_namespace),
                             ModelStates, self.checkForModel)
            r = rospy.Rate(10)
        ...
        # ROS Parameter
            ...
            rospy.logdebug("Loading model xml from ros parameter")
            self.model_xml = rospy.get_param(self.param_name)
            if self.model_xml == "":
        ...
        # Gazebo Model Database
            ...
            rospy.logdebug("Loading model xml from Gazebo Model Database")
            self.model_xml = self.createDatabaseCode(self.database_name)
            if self.model_xml == "":
        ...
        if self.package_to_model:
            self.model_xml = re.sub("<\s*mesh\s+filename\s*=\s*([\"|'])package://",
                               "<mesh filename=\g<1>model://", self.model_xml)
        ...
        tmpq = tft.quaternion_from_euler(self.initial_rpy[0],
                                         self.initial_rpy[1],
                                         self.initial_rpy[2])
        ...
        # spawn model
        if self.urdf_format:
            success = self.gazebo_interface.spawn_urdf_model_client(self.model_name,
                                                               self.model_xml,
            ...
            success = self.gazebo_interface.spawn_sdf_model_client(self.model_name,
                                                              self.model_xml,
                                                              self.robot_namespace,
                                                              initial_pose,
                                                              self.reference_frame,
        ...
        if len(self.joint_names) != 0:
            try:
                success = self.gazebo_interface.set_model_configuration_client(
                    self.model_name, self.param_name,
                    self.joint_names, self.joint_positions,
                    self.gazebo_namespace)
        ...
        if self.unpause_physics:
            rospy.wait_for_service(
                '%s/unpause_physics' % (self.gazebo_namespace))
            try:
